...
def pick_file(files, offset):
    if offset == CLOSEST:
        return sorted(files, key=calculate_offset)[0]
    else:
        return sorted(files, key=lambda x:abs(calculate_offset(x) - offset))[0]
...
file_to_play = pick_file(audio_files, offset_sec)
